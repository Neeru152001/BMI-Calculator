...
def BMI():
    ...
    if bmi<=18.5:
        ...
        label3.config(text='It indicates that a person is slightly \n overweight! A doctor may advise to \n loose some weight')

    else:
        label2.config(text='Obes!')
        label3.config(text='Healthy may be at risk, if they do no \n loose weight!')
...
Height = StringVar()
Weight = StringVar()
height=Entry(root,textvariable=Height,width=5,font='arial 50',bg="#fff",fg="#000",bd=0,justify=CENTER) #text align in center
height.place(x=35,y=160)
Height.set(get_current_value())

weight=Entry(root,textvariable=Weight,width=5,font='arial 50',bg="#fff",fg="#000",bd=0,justify=CENTER) #text align in center
